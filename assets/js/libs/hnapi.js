var date = function () {
	...
	supportXDomainRequest = !!window.XDomainRequest,
...
var req = function (url, success, error) {
	...
	if (supportCORS) {
		if (worker) {
			...
			var r =
				requests[url] || (supportXDomainRequest ? new XDomainRequest() : new XMLHttpRequest());
			...
			if (r.readyState <= 1 || supportXDomainRequest) {
				// XDomainRequest doesn't have readyState
				r.open('GET', url, true);
				r.send();
			}
			requests[url] = r;
