...
var tappable = function (selector, opts) {
	...
	var onMove = function (e) {
		...
		var target = e.target,
			...
			y = e.clientY;
		if (!target || !x || !y) {
			// The event might have a target but no clientX/Y
		...
		if (noScroll) {
			if (
				x > elBound.left - boundMargin &&
				x < elBound.right + boundMargin &&
				y > elBound.top - boundMargin &&
				y < elBound.bottom + boundMargin
			) {
				// within element's boundary
				moveOut = false;
